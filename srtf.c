...
int main() {
    int numberOfProcesses, arrivalTime[20],burstTime[20], completionTime[20], remainingTime[20], turnaroundTime[20], waitingTime[20], tempBurstTime[20];
    ...
    for (currentTime = 0; processCount != numberOfProcesses; currentTime++) {
        ...
        for (int i = 0; i < numberOfProcesses; i++) {
            if (arrivalTime[i] <= currentTime && burstTime[i] > 0) {
                if (smallestPriorityIndex == -1 || burstTime[i] < burstTime[smallestPriorityIndex]) {
